  ...
  for samplesheet in args.samplesheet:
    ...
    with open( samplesheet ) as samplefile:
      ...
      for row in dictreader:
        samplesetnum.update({row['sample_set']:1})
    print ("########### SAMPLES #################")
    print ("SAMPLE SETS AND # SAMPLES: " )     
    for k in samplesetnum:
      print (k, "with", samplesetnum[k], "samples.")
    print ("###########FASTQS / READS #################")
    fastqdir=os.path.dirname(samplesheet) + "/fastq"
    fastqs =os.listdir(fastqdir)
    for sampleset in list(samplesetnum.keys())+['Undetermined'] :
      print (sampleset)
      fqss = [ f for f in fastqs if sampleset in f  and "_R1_" in f ]
      print ( "...   ",  len(fqss)    , " fastqs for R1", sampleset )
      samplereads=collections.Counter()
      #count = 0
      for fq in fqss:
        #print (fq)
        with gzip.open (fastqdir+"/"+fq, mode='rt') as f:
          for line in f:
            if line.startswith("@") :
              samplereads.update({fq: 1})
          #count+=1
          #if count >10: 
          #  break
        #print ("...",f"{samplereads[fq]:,}")
